...
(function($) {
    ...
    if ('content' in document.createElement('template')) {
            ...
            */
            ...
            $.each(json.photos, function (i, photo) {
                if (photo.active === true) {
                    ...
                    template.querySelector('.thumbnail').href = photo.src;
        ...
        $.getJSON(IMAGE_JSON, function (json) {
            // Get a reference to the div anchor in the main DOM.
            var portfolioImages = document.getElementById('portfolio-thumbnails');

            /*
            <template id="portfolio-template-thumb">
                <li class="span4">
                    <div class="thumbnail">
                        <img data-src="" class="img-responsive portfolio-image" alt="">
                        <h3 class="portfolio-title"></h3>
                        <p class="portfolio-media"></p>
                    </div>
                </li>   
            </template>
            */
            ...
            $.each(json.photos, function (i, photo) {
                if (photo.active === true) {
                    var template = document.getElementById('portfolio-template-thumb').content.cloneNode(true);
                    template.querySelector('.portfolio-image').src = photo.src;
                    //template.querySelector('.thumbnail').href = photo.src;
